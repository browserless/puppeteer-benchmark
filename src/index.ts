...
export const aggregateResults = (
	resultItems: TestCasePerformanceResultItem[],
): AggregatedResultItem[] => {
			...
			return Array.from(reportedMeasures).map((measureName) => {
				return Array.from(puppeteerVersions).reduce<AggregatedResultItem>(
					(acc, puppeteerVersion) => {
						...
						const relatedDurations = relatedRunResults.map(
							(_) =>
								_.item.detail?.pdfSize ||
								_.item.detail?.memory ||
								_.item.detail?.cpu ||
								_.item.duration,
						);
 ...
 */
export const printResultsTable = (
